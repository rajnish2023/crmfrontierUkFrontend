...
import { fetchPageById, updatePage } from '../../api/api'
...
const EditPage = () => {
  ...
  useEffect(() => {
    const loadPage = async () => {
      try {
        const res = await fetchPageById(token, id)
        const page = res.data
        setTitle(page.title || '')
        setSlug(page.slug || '')
  ...
  if (loading) {
    return (
      <div style={{ 
        ...
        <p style={{ color: '#64748b', fontSize: '0.95rem' }}>Loading page...</p>
